<template>
  <div class="room-management">
    ...
    <Dialog
      ...
      @hide="onBatchImportComplete"
    >
      <RoomBatchImport @complete="onBatchImportComplete" />
    </Dialog>
    ...
    >
      <RoomDetail v-if="selectedRoom" :room="selectedRoom" />
    </Dialog>
...
const loadRooms = async () => {
  ...
  try {
    ...
    const response = await $fetch('/api/v1/rooms', { params })
...
const exportRooms = async () => {
  ...
  try {
    ...
    const response = await $fetch('/api/v1/rooms/export', { params })
...
const confirmDelete = async () => {
  ...
  try {
    await $fetch(`/api/v1/rooms/${deletingRoom.value.id}`, {
      method: 'DELETE'
    })
