...
const generateTimeSlots = (roomId: string, targetDate: Date): TimeSlot[] => {
  ...
  for (let hour = 8; hour < 18; hour++) {
    for (let minute = 0; minute < 60; minute += 30) {
      const startTime = addMinutes(addHours(targetDate, hour), minute)
...
function handleRoomChange() {
  ...
  message.value = `已选择会议室：${mockRooms.value.find(r => r.id === selectedRoom.value)?.name}`
}
...
// 生命周期
onMounted(() => {
  console.log('✅ 新预约界面测试页面加载成功!')
  message.value = '新布局加载成功！请填写左侧表单并选择时间'
})
</script>
    ...
    <div class="bg-white shadow-sm border-b">
      <div class="container mx-auto px-4 py-6">
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-2xl font-bold text-gray-900">预约界面布局测试</h1>
            <p class="mt-1 text-gray-600">新的左右两列布局演示</p>
          </div>
