<template>
  <div class="detailed-reservation-form">
    ...
    <div class="form-header mb-6">
      <h2 class="text-2xl font-bold text-gray-800 mb-2">详细预约配置</h2>
    ...
    <Card class="mb-6">
      ...
      <template #content>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <!-- 重要性级别 -->
          <div class="field-group">
            <label for="importanceLevel" class="block text-sm font-medium text-gray-700 mb-2">
              重要性级别 <span class="text-red-500">*</span>
            </label>
            <Dropdown
              id="importanceLevel"
            ...
            />
            <small v-if="errors.importanceLevel" class="text-red-500">{{ errors.importanceLevel }}</small>
          </div>
          ...
          <!-- 参会人数 -->
          <div class="field-group">
            <label for="attendeeCount" class="block text-sm font-medium text-gray-700 mb-2">
              预计参会人数 <span class="text-red-500">*</span>
            </label>
            <InputNumber
              id="attendeeCount"
            ...
            />
            <small v-if="errors.attendeeCount" class="text-red-500">{{ errors.attendeeCount }}</small>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="field-group">
            <label class="block text-sm font-medium text-gray-700 mb-2">
              选择时间 <span class="text-red-500">*</span>
            </label>
    ...
    <EquipmentSelector
      ...
      :selected-time-slot="formData.startTime && formData.endTime ? { startTime: formData.startTime, endTime: formData.endTime } : undefined"
...
import type { MaterialFile, EquipmentSelection, EquipmentConflict, ServiceSelection } from '../../../../types/meeting'
...
const validateField = (field: string) => {
  switch (field) {
    ...
    case 'importanceLevel':
      ...
      break
    case 'attendeeCount':
      errors.attendeeCount = formData.attendeeCount && formData.attendeeCount > 0
        ? ''
        : '参会人数必须大于0'
...
const loadTimeSlots = async () => {
  ...
  try {
    // 使用store获取可用时间段
    const availability = await reservationStore.fetchAvailability({
      roomIds: [formData.roomId],
      startTime: new Date(),
      endTime: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) // 未来7天
    })

    timeSlots.value = availability.slots || []
  } catch (error) {
...
const onTimeSlotSelect = (slots: TimeSlot[]) => {
...
const onTimeSelectionChange = (slots: TimeSlot[]) => {
  // 处理时间选择变化
  console.log('时间选择变化:', slots)
}

const onEquipmentSelectionChange = (selection: EquipmentSelection[]) => {
...
const onEquipmentConflictDetected = (conflicts: EquipmentConflict[]) => {
  console.warn('设备冲突检测:', conflicts)
  // 这里可以显示冲突提示给用户
}

const onServiceSelectionChange = (selection: ServiceSelection[]) => {
...
const onServiceCostChange = (totalCost: number, totalDiscount: number) => {
  console.log('服务费用变化:', { totalCost, totalDiscount })
}

const onAttendeesChange = (attendees: any[]) => {
...
const onCapacityWarning = (isOverCapacity: boolean, currentCount: number, capacity: number) => {
  if (isOverCapacity) {
...
// 会议材料处理方法
const handleMaterialsUpload = async (materials: MaterialFile[]) => {
  try {
    ...
    console.log('会议材料上传成功:', materials)
  } catch (error) {
...
const handleMaterialDelete = async (materialId: string) => {
  try {
    ...
    console.log('会议材料删除成功:', materialId)
  } catch (error) {
    console.error('处理会议材料删除失败:', error)
  }
}

const handleMaterialPreview = (material: MaterialFile) => {
  console.log('预览会议材料:', material)
  // 可以在这里添加预览逻辑，比如打开预览对话框
}

const handleMaterialDownload = (material: MaterialFile) => {
  console.log('下载会议材料:', material)
  // 浏览器会自动处理下载
